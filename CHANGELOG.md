...
* [CHANGE/BUGFIX] Fail configuration loading if `api_key` and `api_key_file` are defined at the same time. #2910
* [CHANGE/BUGFIX] Fix the `alertmanager_alerts` metric to avoid counting resolved alerts as active. Also added a new `alertmanager_marked_alerts` metric that retain the old behavior. #2943
* [FEATURE] Add `proxy_url` support for OAuth2 in HTTP client configuration. #3010 (0.33)
...
* [ENHANCEMENT] Add `enable_http2` support in HTTP client configuration. #3010 (0.33)
* [ENHANCEMENT] Add `min_version` support to select the minimum TLS version in HTTP client configuration. #3010 (0.34)
...
* [ENHANCEMENT] UI: Allow to choose the first day of week as Sunday or Monday. #3093
